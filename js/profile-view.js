...
import { markViewed } from "./store.js";
...
const ALLOWED = /^(profiles|wipe-required)\/[A-Za-z0-9._-]+\.mobileconfig$/;
const SKIP = new Set([
  "PayloadType",
  "PayloadVersion",
  "PayloadUUID",
...
function render(profile, xml, path) {
  ...
  const bounce = el(
    `<span class="proof-bounce animate__animated animate__bounce animate__infinite animate__slow">Click here for proof this is not a virus!</span>`
  );
  const sum = document.createElement("summary");
  sum.append(bounce);
  proof.append(sum);
  proof.addEventListener("toggle", () => {
    bounce.classList.toggle("animate__infinite", !proof.open);
    bounce.classList.toggle("animate__bounce", !proof.open);
