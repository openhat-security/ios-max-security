...
function read() {
  try {
    return Object.assign(
      { installs: {}, views: {}, color: "system", style: "app" },
      JSON.parse(localStorage.getItem(KEY) || "{}")
    );
  } catch (e) {
    return { installs: {}, views: {}, color: "system", style: "app" };
  }
}
...
export function markViewed(url) {
  const data = read();
  data.views[url] = Date.now();
  write(data);
}
...
export function wasViewed(url) {
  return Boolean(read().views[url]);
}
...
export const VIEW_DONE = "Viewed — tap to re-read";
